...
def encode_cpu(inputs: Tensor, pool: ThreadPool) -> list[bytes]:
    ...
    return list(
        pool.map(
            lambda sample: zstd.compress(sample.tobytes()),
...
def analyze(
    *,
    ...
    """
    Measure the time complexity and rate of encoding and decoding the representations of a pretrained model using Zstandard.
