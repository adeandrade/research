...
@numba.njit
def encode_bits(cumulative_probability: int, state: int, num_bits: int) -> tuple[int | None, int]:
...
def decode_symbol(
    ...
    probability = cdf[value + 1] - cdf[value]

    state_new, position_new = advance_state(state, position, codes, cumulative_probability, probability, PRECISION)
