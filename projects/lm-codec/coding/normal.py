...
from coding import distribution
...
SQUARE_ROOT_2_RECIPROCAL = 2**-0.5
...
@numba.vectorize
def calculate_standardized_cumulative(values):
    return numba.float32(0.5) * math.erfc(values * numba.float32(-(2**-0.5)))  # type: ignore
...
def calculate_cdfs(
    ...
    multiplier = -1 * scipy.stats.norm.ppf(tail_mass / 2)
    ...
    upper = calculate_standardized_cumulative((0.5 - samples) / scale_table[:, None])
    lower = calculate_standardized_cumulative((-0.5 - samples) / scale_table[:, None])
    pmfs = upper - lower

    tail_masses = 2 * lower[:, 0]
    ...
    offsets = -1 * pmf_centers

    return cdfs, cdf_sizes, offsets


@numba.njit
def calculate_qmf(scale: float, precision: int = PRECISION, tail_mass: float = TAIL_MASS) -> tuple[np.ndarray, int]:
    multiplier = -1 * ppf(tail_mass / 2)
    length = math.ceil(scale * multiplier)
    size = 2 * length + 1

    samples = np.arange(size, dtype=np.float32)
    samples = np.abs(samples - length)

    upper = calculate_standardized_cumulative((0.5 - samples) / scale)
    lower = calculate_standardized_cumulative((-0.5 - samples) / scale)
    pmf = upper - lower

    cdf = distribution.pmf_to_quantized_cdf(pmf, precision)
    qmf = cdf[1:] - cdf[:-1]
