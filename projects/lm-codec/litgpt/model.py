...
import lm_codec.functions as functions
...
class GPT(nn.Module):
    ...
    def predict(
        ...
        input_pos_maxp1: Tensor | None = None,
        lm_head_chunk_size: int = 0,
        return_blocks: set[int] = set(),
        quantize: bool = True,
    ) -> tuple[Tensor, list[Tensor]] | tuple[list[Tensor], list[Tensor]]:
        """
        ...
        if lm_head_chunk_size > 0:
            ...
            outputs = [clamp_head(self.lm_head(x_i)) for x_i in x.split(lm_head_chunk_size, dim=1)]
        else:
            outputs = clamp_head(self.lm_head(x))  # (B, T, padded_vocab_size)
    ...
    def forward(
        ...
        return self.predict(self.embed(idx), input_pos, input_pos_maxp1, lm_head_chunk_size, return_blocks, quantize)

    @classmethod
    def from_name(cls, name: str, **kwargs: Any) -> Self:
        return cls(Config.from_name(name, **kwargs))

    def rope_cache(self, device: torch.device = None) -> tuple[Tensor, Tensor]:
...
class KVCache(nn.Module):
    ...
    def forward(self, input_pos: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> tuple[Tensor, Tensor]:
        """
