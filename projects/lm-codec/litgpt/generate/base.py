...
def next_token(
    ...
    **sample_kwargs: Dict[str, Any],
    ...
    logits, *_ = model(x, input_pos, input_pos_maxp1=input_pos_maxp1)
...
def batched_sample(logits: list[torch.Tensor], kwargs: list[dict]) -> torch.Tensor:
    ...
    return torch.stack(
        [sample(logit, **sample_args).to(dtype=torch.int64) for sample_args, logit in zip(kwargs, logits)],
        dim=0,
    )
...
def batched_next_token(
    ...
    logits_list = [logit.unsqueeze(0) for logit in logits_list]
...
def batched_generate_fn(
    ...
    *,
    ...
    # Yield any remaining tokens
    max_token_lists = max(len(token_list) for token_list in token_lists)
