...
from composable_features.processing import EncodeSegmentationTree


class Cityscapes(Dataset):
    num_classes = len(EncodeSegmentationTree.valid_classes)
    ...
    def is_member(self, member: str) -> bool:
        return member.startswith(os.path.join('leftImg8bit', self.split)) and member.endswith('.png')
    ...
    def get_path(self, directory: str, index: int, suffix: str) -> str:
        ...
        path = os.path.join(directory, self.split, city, f'{prefix}{suffix}')
    ...
    def __getitem__(self, index: int) -> Tuple[Any, ...]:
        ...
        with self.file_fetcher.open_member(image_name) as image_file, \
                self.file_fetcher.open_member(labels_name) as labels_file, \
                self.file_fetcher.open_member(depth_name) as depth_file:

            image = Image.open(image_file)
            labels = Image.open(labels_file)
            depth = Image.open(depth_file)

            if self.transform:
                return self.transform((image, labels, depth))
