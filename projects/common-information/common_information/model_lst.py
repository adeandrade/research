...
class ResidualBottleneckBlock(Sequential):
    def __init__(
        self,
        num_channels: int,
        num_hidden_channels: int | None = None,
        kernel_size: int = 3,
        activation: str = 'elu',
        groups: int = 1,
    ) -> None:
...
def elic_lst_downsample(
    ...
    return Sequential(
        Conv2d(in_channels, num_channels[0], kernel_size, stride=2, padding=padding),
        ResidualBottleneckBlockStack(num_channels[0], num_blocks=num_blocks, activation=activation),
        Conv2d(num_channels[0], num_channels[1], kernel_size, stride=2, padding=padding),
        ResidualBottleneckBlockStack(num_channels[1], num_blocks=num_blocks, activation=activation),
        Conv2d(num_channels[1], num_channels[2], kernel_size, stride=2, padding=padding),
        ResidualBottleneckBlockStack(num_channels[2], num_blocks=num_blocks, activation=activation),
        Conv2d(num_channels[2], out_channels, kernel_size, stride=2, padding=padding),
    )
...
def elic_lst_upsample(
    ...
    return Sequential(
        ConvTranspose2d(in_channels, num_channels[0], kernel_size, 2, padding, 1),
        ResidualBottleneckBlockStack(num_channels[0], num_blocks=num_blocks, activation=activation),
        ConvTranspose2d(num_channels[0], num_channels[1], kernel_size, 2, padding, 1),
        ResidualBottleneckBlockStack(num_channels[1], num_blocks=num_blocks, activation=activation),
        ConvTranspose2d(num_channels[1], num_channels[2], kernel_size, 2, padding, 1),
        ResidualBottleneckBlockStack(num_channels[2], num_blocks=num_blocks, activation=activation),
        ConvTranspose2d(num_channels[2], out_channels, kernel_size, 2, padding, 1),
    )


def shuffle_lst_downsample(
    in_channels: int,
    num_blocks: int = 3,
    activation: str = 'elu',
) -> Sequential:
    return Sequential(
        PixelUnshuffle(2),
        ResidualBottleneckBlockStack(in_channels * 4, num_blocks=num_blocks, activation=activation),
        PixelUnshuffle(2),
        ResidualBottleneckBlockStack(in_channels * 16, num_blocks=num_blocks, activation=activation),
        PixelUnshuffle(2),
        ResidualBottleneckBlockStack(in_channels * 64, num_blocks=num_blocks, activation=activation),
        PixelUnshuffle(2),
    )


def shuffle_lst_upsample(
    in_channels: int,
    num_blocks: int = 3,
    activation: str = 'elu',
) -> Sequential:
