...
from conditional_residual.processing_coco import GenerateTargetSegmentation
from conditional_residual.processing_coco import TargetType, AnnotationType, FilterAndRemapCategories


OutputType = Tuple[Tensor, TargetType]


class COCO(Dataset[OutputType], Sequence[OutputType], ImageDataset):
    image_channels = 3
    ...
    def __init__(
        ...
        self.image_ids = self.get_image_ids(self.coco, split, minimum_image_size, categories, min_area)
    ...
    def has_valid_annotations(
        ...
        """
        If it is empty, there is no annotations.
        ...
        if categories:
            annotations = [
                ...
                if image_object['category_id'] in categories
            ]
    ...
    def get_image_ids(
        ...
        for image_id in sorted(coco.imgs.keys()):
            is_valid = (
                cls.has_valid_size(coco, image_id, minimum_image_size) and
                (split != 'train' or cls.has_valid_annotations(coco, image_id, categories, min_area))
            )
